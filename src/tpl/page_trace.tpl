<style>
    #think_page_trace_tab_cont ol li{
                    ...
                    foreach ($info as $k => $val) {
                        if($key == 'SQL'){
                            if(is_string($val)){
                                ...
                                if(empty($val['file'])){
                                    echo '<li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">' . (is_numeric($k) ? '' : $k.' : ') . htmlentities(print_r($val['sql'].$val['info'],true), ENT_COMPAT, 'utf-8') . '</li>';
                                }else{
                                    echo <<<XML
<li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">
    <details>
        <summary>{$val['sql']} {$val['info']}</summary>
        <p>
            <a href="{$val['jump']}">
                {$val['file']}:{$val['line']}
            </a>
...
XML;
            ...
            echo '<li>' . (is_numeric($k) ? '' : $k.' : ') . '<pre>'.var_export($val,true) . '</pre></li>';
                            }
                        }
                    }
                }
